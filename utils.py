...
class NaNLoss(pl.LightningModule):
    def __init__(self):
        super(NaNLoss,self).__init__()
        
    def forward(self, x, y, weights = None, eps = 1e-5, power = 2, mean = True):
        ...
        if mean:
            if weights is not None:
                denom = ((torch.sum((~is_nan).float() * weights) + eps).detach())
            else:
                denom = ((torch.sum((~is_nan).float()) + eps).detach())
            out_mean = out / denom
...
def MLP(channels):
    return Sequential(*[
        Sequential(Linear(channels[i - 1], channels[i]), torch.nn.SiLU())
...
class BesselBasisLayer(pl.LightningModule):
...
class Envelope(pl.LightningModule):
...
class SphericalBasisLayer(pl.LightningModule):
