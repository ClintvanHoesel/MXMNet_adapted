...
class MXMNet(pl.LightningModule):
    def __init__(self, config: Config, num_spherical=7, num_radial=6, envelope_exponent=5):
        ...
        # self.embeddings = nn.Parameter(torch.ones((7, self.dim)))
        self.embeddings = MLP([config.inp, self.dim])
    ...
    def forward(self, data):
        ...
        # h = torch.index_select(self.embeddings, 0, torch.flatten(x.long()))
        h = self.embeddings(x)
        ...
        for layer in range(self.n_layer):
            h = self.global_layers[layer](h, rbf_g, edge_index_g, p_orbs, d_orbs, pos)
